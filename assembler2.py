...
def flagerror(l):
    ...
    return 0
er=0
for k in range(0,len(l3proper)):
    if(":" in l3proper[k][0]):
        if(flagerror(l3proper[k][1:])==1):
            print("Cannot manipulate value in flag at line "+str(cnt))
            er=1
            break
    if(flagerror(l3proper[k])==1):
        print("Cannot manipulate value in flag at line "+str(cnt))
        er=1
        break
    if(k==len(l3proper)-1):
        if("hlt" not in l3proper[k]):
            print("Halt not in the code")
            er=1
            break
    if("hlt" in l3proper[k]):
        if(k!=len(l3proper)-1):
            print("Halt not the last instruction")
            er=1
            break
    if(k>0 and "var" in l3proper[k] and "var" not in l3proper[k-1]):
        print("Variable not in beginning in line"+str(cnt))
        er=1
        break
    le=errorgen(l3proper[k])
    if(len(le)!=0):
        writeerror(le[0:1],cnt)
        er=1
        break
    cnt=cnt+1
if er==1:
